...
def send_reset_password_instructions(user, origin=None):
...
def update_password(user, password, encode=True, send_email=True):
    """Update the specified user's password

    :param user: The user to update_password
    :param password: The unhashed new password
    """
    hashed_password = hash_password(password) # openagua
    user.password = hashed_password.encode() if encode else hashed_password # openagua
    _datastore.put(user)
    if send_email:
