...
def login():
    ...
    login_user_template = config_value('LOGIN_USER_TEMPLATE')
    _ctx_kwargs = _ctx('login')
    return _security.render_template(login_user_template, login_user_form=form, **_ctx_kwargs)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if form_data.get('g-recaptcha-response', True):

            kwargs = form.to_dict()  # openagua
            ...
            do_flash('Hmmm... Are you really a robot?', category='warning')
