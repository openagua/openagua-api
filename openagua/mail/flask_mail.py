...
class APIConnection(object):
    ...
    def send(self, message, envelope_from=None):
        ...
        if not self.mail.suppress:
            resp = requests.post(
                self.mail.endpoint,
                auth=("apis", self.mail.key),
                data={"from": sanitize_address(envelope_from or message.sender),
                      "to": list(sanitize_addresses(message.send_to)),
    ...
    def connect(self):
        ...
        try:
            if self.service == 'SMTP':
                return SMTPConnection(app.extensions['mail'])
            elif self.service == 'API':
                return APIConnection(app.extensions['mail'])
            else:
                return None
        except KeyError:
...
class _APIMail(_MailMixin):
    def __init__(self, endpoint, key, default_sender, debug, max_emails, suppress,
                 ascii_attachments=False):
        self.service = 'API'
...
class Mail(_MailMixin):
    ...
    def init_mail(self, config, debug=False, testing=False):
        ...
        elif config.get('MAIL_API_ENDPOINT'):
            return _APIMail(
                config.get('MAIL_API_ENDPOINT'),
