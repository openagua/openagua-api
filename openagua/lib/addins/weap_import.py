...
from openagua.lib.utils import readfromzip, shapefile2geojson, get_resource_name, coords_to_string

from openagua.security import current_user
from openagua.lib.files import bulk_upload_data
from openagua.lib.templates import get_default_types, prepare_template_for_import
from openagua.lib.networks import add_network, make_node

from openagua.lib.misc import striprtf
...
def import_from_weap(conn, file, project_id, network_name=None, template_name=None):
    # template_id = get_templates(g.conn, current_app.config['ENABLED_TEMPLATES'])[0].id

    storage_location = current_app.config.get('NETWORK_FILES_STORAGE_LOCATION')
    bucket_name = current_app.config.get('AWS_S3_BUCKET')

    # prepare template
    template = conn.call('get_template_by_name', template_name)
    ...
    while True:
        ...
        template = conn.call('add_template', new_template)
        if template.get('id'):
            break
        elif i <= 50:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        network = weap_to_hydra(conn, dir, filename, project_id, network_name, template, type_lookup_path,
...
def weap_to_hydra(conn, dir, filename, project_id, network_name, template, type_lookup_path,
    ...
    for ka in area.key_assumptions:
        ...
        if ta:
            ...
            attr = conn.call('add_attribute', ka['Name'], 'dimensionless')
    ...
    if not network_type['id']:
        ...
        network_type = conn.call('add_templatetype', network_type)
        template['types'] = [network_type]
    else:
        if new_tattrs:
            network_type['typeattrs'].extend(new_tattrs)
            network_type = conn.call('update_templatetype', network_type)
    ...
    for typeid, user_variables in area.UserVariables.items():
        ...
        for v in user_variables:
            attr = conn.call('add_attribute', v['DisplayLabel'], 'dimensionless')
        ...
        if new_tattrs:
            ...
            updated_type = conn.call('update_templatetype', ttype)
    ...
    network = add_network(conn, net, location=storage_location, template_id=template.id, add_baseline=False,
