...
from openagua import db, socketio
from openagua.lib.runners import run_model_rabbitmq, run_model_local, run_model_ec2
from openagua.models import Ping, Run
from openagua.utils import get_utc, get_model, get_network_model
...
def get_all_data_scenarios(conn, network_id):
    data_scenarios = conn.call('get_scenarios', {'network_id': network_id})
    all_scenarios = {
        'option': [],
        'portfolio': [],
        'scenario': [],
    ...
    for data_scenario in data_scenarios:
        if data_scenario['name'] == app.config['DEFAULT_SCENARIO_NAME']:
...
def add_run(sid, model_id, layout=None):
...
def add_ping(sid, status, **data):
    canceled_ping = get_ping(sid, status=ProcessState.CANCELED)
    if canceled_ping:
    ...
    db.session.merge(ping)
    db.session.commit()
    return ping


def get_ping(sid, status=None, last=True):
    if status:
        return Ping.query.filter_by(sid=sid, status=status).first()
    elif last:
        return Ping.query.filter_by(sid=sid).first()
    else:
        return Ping.query.filter_by(sid=sid).last()


def get_last_ping(sid):
    return Ping.query.filter_by(sid=sid).last()


def get_pings(source_id=None, network_id=None):
...
def start_model_run(conn, network_id, guid, config, scenarios, computer_id=None):
    ...
    # model
    network_model = get_network_model(url=conn.url, network_id=network_id)
    ...
    model_kwargs = dict(
        ...
        pubnub_publish_key=app.config.get("PUBNUB_PUBLISH_KEY"),
        pubnub_subscribe_key=app.config.get("PUBNUB_SUBSCRIBE_KEY")
    )
    ...
    for i, scids in enumerate(scenario_ids):
        ...
        add_ping(sid, ProcessState.REQUESTED, **ping_data)
    ...
    if service == 'amqp':
        run_model_rabbitmq(model, run_key, model_kwargs)
...
def publish_model_run_state(sid, state):
    ...
    model = get_model(id=run.model_id)
    ...
    app.pubnub.publish().channel(queue_name).message(message).pn_async(publish_callback)
...
def get_run(sid):
    run = Run.query.filter_by(sid=sid).first()
    return run
...
def get_run_records(source_id=None, network_id=None):
    rows = get_pings(source_id=source_id, network_id=network_id)
    sids = [row.sid for row in rows]
    recs = Ping.query.filter(Ping.sid.in_(sids)).all() if sids else []
...
def delete_run_record(sid):
    Ping.query.filter_by(sid=sid).delete()
    db.session.commit()


def delete_run_records(source_id=None, network_id=None):
    rows = get_pings(source_id=source_id, network_id=network_id)
    sids = [row.sid for row in rows]
    for sid in sids:
        Ping.query.filter_by(sid=sid).delete()
...
def end_model_run(sid, status, data, report_to_browser=False):
    data.pop('sid', None)
    data.pop('status', None)
    ping = add_ping(sid, status, **data)
...
def emit_progress(source_id=None, network_id=None, ping=None):
    if network_id:
        room = app.config['NETWORK_ROOM_NAME'].format(source_id=source_id, network_id=network_id)
        event = 'network-run'
    else:
        room = app.config['RUN_STUDY_ROOM_NAME'].format(source_id)
