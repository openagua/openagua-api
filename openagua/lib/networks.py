...
def add_network(conn, net, location=None, template_id=None, add_baseline=True, start_time=None, end_time=None,
    ...
    i = 0
    while True:
        result = conn.call('add_network', net, return_summary=return_summary, timeout=10)
    ...
    if network and add_baseline:
        ...
        if not baseline:
            baseline = add_default_scenario(network['id'], start_time=start_time, end_time=end_time,
                                            time_step=time_step)
...
def repair_network(conn, source_id, network_id=None, network=None, options=None):
    ...
    if 'types-attrs' in options:
        network, template = change_active_template(conn, source_id, network=network)
        ...
        if junction_type:
            ...
            for (x, y), old_nodes in positions.items():
                if len(old_nodes) > 1:
                    ...
                    new_node = conn.add_node(network_id, junction)
                    ...
                    for old_node in old_nodes:
                        ...
                        new_links = update_links2(conn, old_node_id=old_node.id, new_node_id=new_node.id,
                                                  old_link_ids=old_link_ids)
                        all_new_links.extend(new_links)
                        conn.call('delete_node', old_node.id, False)
    ...
    if 'scenarios' in options:
        ...
        if not baseline:
            if network.scenarios:
                ...
                baseline = add_default_scenario(network.id)
        else:
            baseline = baseline[0]
        ...
        for scenario in network['scenarios']:
            ...
            if scenario_class in ['option', 'scenario']:
                ...
                if original_parent_id != ancestor_id:
                    ...
                    conn.call('update_scenario', scen=scenario)

    # remove loop links
    if 'topology' in options:
        for link in network['links']:
            if link['node_1_id'] == link['node_2_id']:
                conn.call('delete_link', link['id'], False)
...
def add_default_scenario(network_id, start_time=None, end_time=None, time_step=None):
    scenario = {
        'name': current_app.config['DEFAULT_SCENARIO_NAME'],
        'description': current_app.config['DEFAULT_SCENARIO_DESCRIPTION'],
        'layout': {'class': 'baseline'},
        'start_time': start_time,
        'end_time': end_time,
        'time_step': time_step,
    }
    result = g.conn.call('add_scenario', network_id, scenario)
    return result
...
def add_links_from_geojson(conn, network, template, gj, existings):
    ...
    for i, [x, y] in enumerate(coords):
        ...
        if i and is_new:
            new_cnt += 1
            node2 = node
            name, description = link_name_description(network, link_name, link_type.name, node1, node2, segment=new_cnt)
    ...
    new_links = conn.call('add_links', network.id, new_links)
...
def move_network(source, destination, project_id, network_id):
    ...
    if destination == source:
        ...
        network = g.conn.call('get_network', network_id, summary=False, include_resources=True,
                              include_data=False)
        network['project_id'] = project_id
        template_id = network.layout.get('active_template_id')
        ...
        old_template = None
        if template_id:
            old_template = g.conn.call('get_template', template_id)
        ...
        new_network = add_network(conn=g.conn, net=cleaned_network, template_id=new_template_id)
...
def clone_network(conn, network_id, **kwargs):
    ...
    net = get_filtered_network(conn, network_id, **kwargs)
    ...
    if duplicate_template:
        if template_id:
            old_template = conn.call('get_template', template_id)
    ...
    # add the network
    new_network = add_network(g.conn, net)

    new_network = g.conn.call('get_network', new_network['id'], include_data=True)
    ...
    newer_network = conn.call('update_network', new_network)
...
def get_filtered_network(conn, network_id, **kwargs):
    ...
    if include_data and include_input and include_results:
        network = conn.call('get_network', network_id, include_data=include_data,
                            include_resources=True)
    elif include_data:
        scenario_ids = []
        network = conn.call('get_network', network_id, include_data=False, include_resources=False,
                            summary=False)
        if include_input:
            ...
            include_data = False
        network = conn.call('get_network', network_id, include_data=include_data, include_resources=True,
...
def get_network_for_export(conn, network_id, options, file_format):
    # QC the template
    ...
    if file_format == 'adjacency':
        ext = 'csv'
        network = conn.call('get_network', network_id, include_data=False, include_resources=True,
                            summary=True)
        ...
        network = get_filtered_network(conn, network_id, **options)
...
def save_network_preview(network, filename, contents, location, s3=None):
...
def import_from_json(conn, file, project_id):
    ...
    if net:
        ...
        network = add_network(conn, net=net, template_id=template_id)
...
def update_types(resource, resource_type):
    tpl_ids = []
    updated_types = []
    for type in resource['types'][::-1]:
        if type['template_id'] in tpl_ids:
            g.conn.call('remove_type_from_resource', type_id=type['id'], resource_type=resource_type,
...
def add_update_network_settings(user_id, source_id, network_id, settings):
    ...
    if not network_settings:
        network_settings = UserNetworkSettings(user_id, network_id, settings)
