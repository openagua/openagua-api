...
from openagua.lib.files import upload_network_data, add_storage, s3_bucket, object_url, s3_object_summary
...
def make_geojson_from_node(node, template, template_type=None, icon=True):
    ...
    properties = {
        'id': node.id,
        'name': node.name,
        'displayname': node.layout.displayname if 'displayname' in node.layout else node.name,
    ...
    if icon:
        if 'image' in ttype.layout:
            gj['properties']['image'] = ttype.layout.image
...
def make_geojson_from_link(network, link, template, template_type=None, icon=True):
    ...
    if gj is not None:
        ...
        gj['properties'].update({
            'id': link.id,
            'name': link.name,
            'displayname': link.layout.displayname if 'displayname' in link.layout else link.name,
...
def make_links_from_geojson(conn, network, template, gj, existings, split_locs):
    ...
    for i, [x, y] in enumerate(coords):
        ...
        if node_type and not existing.nodeId or make_junction:
            ...
            node = conn.add_node(network.id, node)
            node['layout']['geojson'] = make_geojson_from_node(node, template)
            node = conn.call('update_node', node)
            node = conn.get_node(node.id)

            new_gjs.append(node['layout']['geojson'])

            if make_junction:
                new_links = update_links(conn, network, existing.nodeId, node.id)
                conn.call('delete_node', existing.nodeId, True)
            ...
            node = conn.get_node(node_id=existing.nodeId)
        ...
        if i and is_new:
            ...
            node_2 = node
            new_link_gj = make_link(conn, network.id, template, new_gj, node_1, node_2, segment=new_cnt)
    ...
    for line_id, splits in split_locs.items():
        ...
        divided_gjs = split_link_at_nodes(conn=conn, template_id=template.id, network_id=network.id,
...
def add_link(conn, network, template, ttypes, incoming_link, existings, split_locs, default_types, del_nodes=[]):
    ...
    for i, [x, y] in enumerate(coords):
        ...
        if node_type and not existing.get('nodeId') or make_junction:  # make new inflow, outflow or junction node
            ...
            if replace_existing_node:
                ...
                _new_links = update_links(conn, network, old_node_id=old_node_id, new_node_id=node.id)
                new_links.append(_new_links)
                conn.call('delete_node', old_node_id, True)
                # network['nodes'] = [n for n in network.nodes if n.id != old_node_id]
                del_nodes.append(old_node_id)

        else:
            node = conn.call('get_node', existing.get('nodeId'))
        ...
        if i and is_new:
            ...
            new_link = conn.add_link(network.id, _new_link)
    ...
    for line_id, splits in split_locs.items():
        ...
        _new_links = split_link_at_nodes2(conn=conn, network_id=network.id, template_id=template.id,
...
def split_link_at_nodes(conn, network_id, template_id, link_id=None, nodes=None, splits=None):
    link = conn.call('get_link', link_id)

    new_gj = []

    # split geojson - find the new coordinates
    node_1 = conn.get_node(node_id=link.node_1_id)
    for i, split in enumerate(splits):
        ...
        if idx is not None:
            ...
            node_2 = conn.get_node(node_id=link.node_2_id)
        nl = dict(
        ...
        new_link = conn.add_link(network_id, nl)
        ...
        new_link = conn.call('update_link', new_link)
        new_link = conn.get_link(new_link.id)
        new_gj.append(deepcopy(gj))

        if idx is not None:
            node_1 = nodes[idx]
    conn.call('delete_link', link.id, True)

    return new_gj


def split_link_at_nodes2(conn, network_id, template_id=None, old_link_id=None, nodes=None, splits=None):
    link = conn.get_link(old_link_id)
    new_links = []

    types = []
    for rt in link.types:
        tt = rt.templatetype
        types.append(JSONObject({
            'id': tt.id,
            'template_id': template_id
        }))
    ...
    # split geojson - find the new coordinates
    # node_1 = conn.get_node(node_id=link.node_1_id)
    node_1 = conn.call('get_node', link.node_1_id)
    for i, split in enumerate(splits):
        idx = split['idx']
        # add a new link
        # new_link = link.copy()
        if idx is not None:
            ...
            node_2 = conn.call('get_node', link.node_2_id)
        ...
        while True and j <= 100:
            ...
            if i == 0:
                new_link = conn.call('update_link', nl)
            else:
                new_link = conn.add_link(network_id, nl)
...
def make_link(conn, network_id, template, gj, node_1, node_2, segment=None):
    ...
    if len(lname):
        ...
        node_1_name = conn.get_node(node_1_id).name
        node_2_name = conn.get_node(node_2_id).name
        lname = '{} - {} to {}'.format(type_name, node_1_name, node_2_name)
        link['description'] = '{} from {} to {}'.format(type_name, node_1_name, node_2_name)
    link['id'] = None

    network = conn.call('get_network', network_id, include_data=False)
    ...
    # add link to db
    new_link = conn.add_link(network_id, link)
    ...
    gj['properties']['id'] = new_link.id
    new_link['layout'] = {'geojson': gj}
    conn.call('update_link', new_link)
...
def purge_replace_feature(conn, feature, network, template):
    if feature.geometry.type == 'Point':
        error, new_gj, del_links = purge_replace_point(conn, network, feature, template)
    else:
        error, new_gj, del_links = purge_replace_line(conn, feature)
    return error, new_gj, del_links


def purge_replace_features(conn, features, network, template):
    error = 0
    new_gj_all = []
    del_links_all = []
    for polyline in features.polylines:
        error, new_gj, del_links = purge_replace_line(conn, polyline)
        if error:
            break
        else:
    ...
    if not error:
        network = conn.call('get_network', network.id)
        for point in features.points:
            error, new_gj, del_links = purge_replace_point(conn, network, point, template)
            if error:
                break
            else:
                new_gj_all.extend(new_gj)
...
def purge_replace_line(conn, gj):
    error = 0
    new_gjs = []
    del_links = []

    link_id = gj.properties.id
    result = conn.call('delete_link', link_id, True)
    if result != 'OK':
        result = conn.call('delete_link', link_id, False)
    if result != 'OK':
        error = 1
    else:
        # load_active_study(conn)
        del_links = [link_id]

    return error, new_gjs, del_links


def purge_replace_point(conn, network, gj, template):
    error = 0
    new_gjs = []
    new_node, new_links, del_links = purge_replace_node(conn, network, template, gj)
    if new_node:
...
def purge_replace_node(conn, network, template, gj):
    ttypes = {ttype.name: ttype for ttype in template.templatetypes}
    ...
    if type_name == 'Junction':
        ...
        if len(adj_links) == 2 \
            ...
            # new_link = conn.call('update_link', uplink)
            # conn.call('delete_link', downlink, False)
            new_links = update_links(conn, network, node_id)
        ...
        if adj_links:
            ...
            node = make_generic_node(template.id, ttype.id, node_name, x, y)
            new_node = conn.add_node(network.id, node)
            ...
            #     new_links = update_links(conn, network, node_id)
            # else:
            new_links = update_links(conn, network, node_id, new_node.id)
    ...
    conn.call('delete_node', node_id, True)

    return new_node, new_links, del_links


def update_links(conn, network, old_node_id, new_node_id=None):
    updated_links = []
    ...
    if new_node_id is not None:
        for link in network.links:
            if old_node_id in [link.node_1_id, link.node_2_id]:
                ...
                updated_link = conn.call('update_link', new_link)
        ...
        conn.call('delete_link', old_link['id'], False)
        updated_link = conn.call('update_link', new_link)
        updated_links.append(updated_link)
    return updated_links


def update_links2(conn, old_node_id, new_node_id=None, old_link_ids=[]):
...
def get_network_references(conn, network, update_url=True):
    refs = []
...
def repair_network_references(network, update_url=True):
    ...
    location = storage.get('location', current_app.config['NETWORK_FILES_STORAGE_LOCATION'])
...
def add_network_reference(conn, network, reference, geojson):
...
def delete_network_references(conn, network_id, reference_ids):
    network = conn.call('get_network', network_id, include_resources=False, include_data=False,
                        summary=True)

    keepers = []
    if 's3' in network.layout.storage.location.lower():
        bucket = s3_bucket(current_app.config['AWS_S3_BUCKET'])
        folder = network.layout.storage.folder
